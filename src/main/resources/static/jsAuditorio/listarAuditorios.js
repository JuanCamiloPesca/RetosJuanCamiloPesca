function listarAuditorios(){    
    $.ajax({
        ...
        success:function (respuesta){
            ...
            swal ( "Listar auditorio" ,  "Se ha actualizado Listado!" ,  "success" );
        },
...
/* Se deshabilita la funcion disable btnBorrarAuditorio
function disablebtnBorrarAuditorio(){
    $.ajax({
        url:"https://g318379ad1c6e62-reto1.adb.sa-santiago-1.oraclecloudapps.com/ords/admin/audience/audience",
        type:'GET',
        dataType:'json',

        success:function (respuesta){
            for (let i = 0; i < respuesta.items.length; i++){    
                $("#btnBorrarAuditorio"+respuesta.items[i].id).prop('disabled',true);
            }
        },
...
function listarRespuestaAuditorios(items){
    ...
    for (let i = 0; i < items.length; i++){
        ...
        tabla+=`<tr>
                    ...
                    <td><button  id="btnEditarAuditorio${items[i].id}">Editar</button> </td>
                    <td><button  id="btnBorrarAuditorio${items[i].id}"}>Borrar</button> </td>
                </tr>`;
    }
    tabla+=`</table>`
